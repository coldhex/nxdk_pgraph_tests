...
static std::string MakeTestName(bool perspective_corrected, bool flat, float rotate_angle, int vertex_cycle) {
  ...
  return std::string(kTestClippingPrecision) + "_nopers" + (perspective_corrected ? "0" : "1") +
    + "_flat" + (flat ? "1" : "0") + rot + vert;
}

ClippingPrecisionTests::ClippingPrecisionTests(TestHost &host, std::string output_dir) : TestSuite(host, std::move(output_dir), "Clipping precision") {
...
void ClippingPrecisionTests::Deinitialize() {
  TestSuite::Deinitialize();
}

void ClippingPrecisionTests::TestClippingPrecisionFrame(float ofs,
...

  {
    ...
    float tri_vertices[] = {
      480.0, 100.0, 30000.0, tu, 0.0,
      480.0, 240.0, 30000.0, tu, 0.5,
      -15000000.0f/w2, -2500000.0f/w2, w2, 0.0, 0.0,
      480.0, 240.0, 30000.0, tu, 0.0,
      480.0, 380.0, 30000.0, tu, 0.5,
      -15000000.0f/w2, 2500000.0f/w2, w2, 0.0, 0.0,
    };
  ...
  host_.FinishDraw(false, output_dir_, test_name);
...
void ClippingPrecisionTests::TestClippingPrecision(bool perspective_corrected,
  ...
  for (int i = 0; i < num_frames; i++) {
    TestClippingPrecisionFrame(ofs, perspective_corrected, flat, rotate_angle,
                               vertex_cycle, i == num_frames - 1);
    ofs += 0.001;
  }
}
