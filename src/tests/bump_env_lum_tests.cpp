...
BumpEnvLumTests::BumpEnvLumTests(TestHost &host, std::string output_dir)
    : TestSuite(host, std::move(output_dir), "Bump env lum tests") {
  ...
  for (auto nv_texture_format : {NV097_SET_TEXTURE_FORMAT_COLOR_SZ_R6G5B5, NV097_SET_TEXTURE_FORMAT_COLOR_SZ_X8R8G8B8}) {
...
void BumpEnvLumTests::Test(const TextureFormatInfo &texture_format, bool cross_on_blue, bool rotate90) {
  ...
  for (auto gsigned : {0, 1}) {
	...
	stage.SetFilter(0, TextureStage::K_QUINCUNX, TextureStage::MIN_TENT_LOD0, TextureStage::MAG_TENT_LOD0,
			asigned, rsigned, !!gsigned, !!bsigned);
      }
