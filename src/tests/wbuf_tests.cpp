...
static bool IsWBuf(int depthf) {
  return depthf & 4;
}

static int GetDepthBits(int depthf) {
  return (depthf & 2) ? 24 : 16;
}

static bool IsFloatDepth(int depthf) {
  return depthf & 1;
}
...
static std::string MakeTestName(const char* prim_name, int depthf, bool zbias, bool zslope, bool vsh,
  ...
  char depth_buf[32] = {0};
  snprintf(depth_buf, sizeof(depth_buf), "%s%d%c", wbuf ? kTestWBuf : kTestZBuf, depth_bits,
           float_depth ? 'F' : 'D');
  ...
  return std::string(depth_buf) + "_" + prim_name + clip_buf + "_V" + (vsh ? "1" : "0") +
    "_ZB" + (zbias ? "1" : "0") + "_ZS" + (zslope ? "1" : "0");
}

WBufTests::WBufTests(TestHost &host, std::string output_dir) : TestSuite(host, std::move(output_dir), "W buffering") {
  for (auto depthf : {0, 1, 2, 3, 4, 5, 6, 7}) {
    for (auto zbias : {false, true}) {
      for (auto zslope : {false, true}) {
        bool vsh = true;

        tests_[MakeTestName(kPrimFloorQuad, depthf, zbias, zslope, vsh)] =
          [this, depthf, zbias, zslope, vsh]() {
        ...
        tests_[MakeTestName(kPrimRoofQuad, depthf, zbias, zslope, vsh)] =
          [this, depthf, zbias, zslope, vsh]() {
        ...
        tests_[MakeTestName(kPrimWallQuad, depthf, zbias, zslope, vsh)] =
          [this, depthf, zbias, zslope, vsh]() {
        ...
        tests_[MakeTestName(kPrimLargeZ, depthf, zbias, zslope, vsh)] =
          [this, depthf, zbias, zslope, vsh]() {
        ...
        tests_[MakeTestName(kPrimLineStrip, depthf, zbias, zslope, vsh)] =
          [this, depthf, zbias, zslope, vsh]() {
        ...
        tests_[MakeTestName(kPrimHTri, depthf, zbias, zslope, vsh)] =
          [this, depthf, zbias, zslope, vsh]() {
        ...
        tests_[MakeTestName(kPrimVTri, depthf, zbias, zslope, vsh)] =
          [this, depthf, zbias, zslope, vsh]() {
...

  {
    ...
    tests_[MakeTestName(kPrimRiddick, depthf, zbias, zslope, vsh)] =
      [this, depthf, zbias, zslope, vsh]() {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    tests_[MakeTestName(kPrimFloorClip, depthf, zbias, zslope, vsh, clip_left, clip_top)] =
      [this, depthf, zbias, zslope, vsh, clip_left, clip_top]() {
        this->Test(depthf, zbias, zslope, vsh, kPrimFloorClip, [this]() {
          float tu = 1.0;
  ...
  for (int i = 0; i < 3; i++) {
    ...
    tests_[MakeTestName(kPrimWallClip, depthf, zbias, zslope, vsh, clip_left, clip_top)] =
      [this, depthf, zbias, zslope, vsh, clip_left, clip_top]() {
        this->Test(depthf, zbias, zslope, vsh, kPrimWallClip, [this]() {
          float tu = 1.0;
...
void WBufTests::Initialize() {
  TestSuite::Initialize();
}

void WBufTests::Deinitialize() {
...
void WBufTests::Test(int depthf, bool zbias, bool zslope, bool vsh, const char* prim_name, Func draw_prim,
  ...
  p = pb_push1(p, NV097_SET_CONTROL0, 0x100000 | (wbuf ? NV097_SET_CONTROL0_Z_PERSPECTIVE_ENABLE : 0) |
