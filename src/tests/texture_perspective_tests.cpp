...
TexturePerspectiveTests::TexturePerspectiveTests(TestHost &host, std::string output_dir) : TestSuite(host, std::move(output_dir), "Texture perspective") {
  for (auto perspective_corrected : {false, true}) {
    for (auto quad : {false, true}) {
      tests_[MakeTexPersTestName(quad, perspective_corrected)] =
	[this, quad, perspective_corrected]() { this->TestTexturePerspective(quad, perspective_corrected); };
      tests_[MakeTexDiffTestName(quad, perspective_corrected)] =
	[this, quad, perspective_corrected]() { this->TestDiffusePerspective(quad, perspective_corrected); };
    }
  }
}
...
void TexturePerspectiveTests::Deinitialize() {
...
void TexturePerspectiveTests::TestTexturePerspective(bool draw_quad, bool perspective_corrected) {
  ...
  host_.FinishDraw(allow_saving_, output_dir_, MakeTexPersTestName(draw_quad, perspective_corrected));
...
void TexturePerspectiveTests::TestDiffusePerspective(bool draw_quad, bool perspective_corrected) {
  ...
  host_.FinishDraw(allow_saving_, output_dir_, MakeTexDiffTestName(draw_quad, perspective_corrected));
